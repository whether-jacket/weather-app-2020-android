...
import com.seljabali.core.network.ResponseHeaders
...
class ResponseCookiesInterceptor(private val listener: CookieListener) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (originalResponse.headers(ResponseHeaders.SET_COOKIE.id).isNotEmpty()) {
            val cookies = HashSet<String>()
            for (header in originalResponse.headers(ResponseHeaders.SET_COOKIE.id)) {
                cookies.add(header)
            }
